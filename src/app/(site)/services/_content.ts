...
export const FALLBACK_SERVICES: readonly ServiceItem[] = [
  ...
  fallback(
    ...
    'Khi thiết kế và thi công nằm chung một đầu mối, khoảng cách giữa bản vẽ và hiện trường gần như biến mất. Mẫu được duyệt tại xưởng. Sai số xử lý trong tuần, chứ không để dồn thành một cuộc thương lượng ở cuối dự án.\n\nMỗi tuần một buổi ở công trường, và một biên bản có ảnh gửi về trong ngày. Phát sinh được báo trước khi làm, kèm con số, chứ không phải sau khi đã trót làm.',
  ),
  fallback(
    4,
    'tu-van-vat-lieu',
...
/**
 * The paragraph most studio sites leave out. Deliberately uneven — one of these
 * is a single clause, one runs to a single long sentence, and no two of them
 * open the same way.
 */
export const EXCLUSIONS: readonly string[] = [
  'Chưa đi xem hiện trạng thì chúng tôi chưa nhận việc. Ảnh chụp bằng điện thoại không nói được trần cao bao nhiêu, cũng không nói được ba giờ chiều nắng vào tới đâu trên sàn.',
  'Nếu bạn đã có sẵn một thư mục ảnh và chỉ cần người dựng lại cho giống, việc đó có người làm nhanh hơn và rẻ hơn chúng tôi rất nhiều. Nói vậy không phải để chê ai. Chỉ là chúng tôi làm việc đó dở.',
  'Studio không ký tên vào phương án do người khác vẽ.',
  'Chúng tôi cũng không nhận thi công cho bản vẽ không phải của mình. Không phải để giữ chỗ. Chỉ là không đứng ra bảo hành được cho những chi tiết mình chưa từng ngồi tính.',
  'Không có gói “thiết kế miễn phí khi thi công” — bản vẽ là phần tốn thời gian nhất trong toàn bộ công việc này, nên gói nó vào giá thi công chỉ có nghĩa là bạn vẫn đang trả tiền cho nó ở một dòng khác, chỗ bạn không nhìn thấy.',
