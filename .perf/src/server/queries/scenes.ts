...
/**
 * Every scene's mode, keyed by project — the whole (tiny) table in one
 * statement.
 *
 * Deliberately unfiltered, and deliberately NOT wrapped in `cache()`. A listing
 * needs the modes for the projects it is about to return, which normally means
 * waiting for the project query before the scene query can even be issued: two
 * serial round trips to Neon, ~250 ms each from Saigon. Asking for all of them
 * removes the dependency, so the two statements go out together and the listing
 * pays one hop instead of two. `scenes` holds one row per scene (27 today,
 * bounded by the project count) and this reads two small columns of it.
 *
 * Raw rather than `cache()`d because the only caller runs inside
 * `unstable_cache`, where a React request-scoped memo has nothing to memoise
 * against. First row per project wins, matching `getSceneForProject`.
 */
export async function readAllSceneModes(): Promise<Map<string, SceneMode>> {
  const rows = await db
