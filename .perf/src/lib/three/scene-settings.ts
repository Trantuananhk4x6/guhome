/**
 * Scene configuration helpers — defaulting, validation and waypoint maths
 * shared by the scene components. Pure, and deliberately free of any `three`
 * import: the light shell of `InteriorScene` reads scene settings on every
 * visit to decide whether a canvas is warranted at all, and one renderer
 * constant reached from here would drag 861 KB of three.js onto the arrival
 * path. Everything that genuinely needs three lives in `gl-settings.ts`.
 */
