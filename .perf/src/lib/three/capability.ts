...
/**
 * The visitor has asked for less, or is on a link that cannot afford more.
 *
 * `Save-Data: on` is an explicit request; `2g` / `slow-2g` is the browser's own
 * estimate of a link on which 1.2 MB of renderer is measured in whole seconds.
 * Either way the photograph is not a degraded version of this site, it *is* the
 * site — so the scene is never fetched rather than fetched and then discarded.
 *
 * Deliberately separate from {@link devicePerf}: that answers "can this device
 * draw the scene well", which a fast laptop on hotel wifi answers yes to.
 */
